...
function GratiaLogo() {
  return (
    ...
    >
      {/* NO rect — transparent background */}
      <text x="60" y="250"
        fontFamily="'Arial Black','Arial Narrow',Arial,sans-serif"
        fontSize="180" fontWeight="900"
        fill="#1a1a2e" letterSpacing="-8">GRAT</text>
      <text x="554" y="250"
        fontFamily="'Arial Black','Arial Narrow',Arial,sans-serif"
        fontSize="180" fontWeight="900"
        fill="#ff3b30" letterSpacing="-8">IA</text>
      <text x="720" y="250"
        fontFamily="Arial,sans-serif"
        fontSize="180" fontWeight="100"
        fill="#1a1a2e"
        letterSpacing="-8"> CORE</text>
    </svg>
  )
}
...
function QuickSignupModal({ onClose }) {
    ...
    <div style={{position:'fixed',inset:0,zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.45)',backdropFilter:'blur(10px)',padding:24}}>
      <div style={{background:'#fff',borderRadius:26,padding:'36px 32px',maxWidth:420,width:'100%',boxShadow:'0 32px 80px rgba(0,0,0,.18)',fontFamily:"'DM Sans',system-ui,sans-serif",position:'relative',animation:'modalPop .35s cubic-bezier(.34,1.56,.64,1)'}}>
        ...
        {step === 'gate' && <>
          ...
          <h2 style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:800,letterSpacing:-.5,textAlign:'center',color:'#1a1a2e',marginBottom:8}}>See gas prices near you</h2>
          <p style={{fontSize:14,color:'rgba(26,26,46,.55)',textAlign:'center',lineHeight:1.65,marginBottom:22}}>Free account — no credit card. Real prices in 30 seconds.</p>
          <div style={{background:'#f8f7fc',borderRadius:14,padding:'14px 16px',marginBottom:22}}>
            {[{icon:'📍',text:'Real-time prices at nearby stations'},{icon:'🛣️',text:'Cheapest gas on any route you drive'},{icon:'🧾',text:'IRS mileage deduction calculator'},{icon:'🔔',text:'Alerts when prices drop near you'}].map((f,i)=>(
          ...
          <input type="email" placeholder="Your email address" value={email} onChange={e=>{setEmail(e.target.value);setError('')}} onKeyDown={e=>e.key==='Enter'&&handleContinue()} autoFocus style={{width:'100%',padding:'13px 16px',background:'#f8f7fc',border:`1.5px solid ${error?'#ff453a':'rgba(0,0,0,.1)'}`,borderRadius:14,fontSize:15,color:'#1a1a2e',outline:'none',fontFamily:"'DM Sans',sans-serif",marginBottom:error?6:10}}/>
          ...
          <p style={{fontSize:11,color:'rgba(26,26,46,.3)',textAlign:'center',marginTop:14,lineHeight:1.6}}>No credit card · Free forever · <a href="/privacy" style={{color:'rgba(26,26,46,.4)'}}>Privacy</a></p>
        </>}
        ...
        {step === 'details' && <>
          ...
          <p style={{fontSize:13,color:'rgba(26,26,46,.5)',marginBottom:20}}>Creating account for <strong style={{color:'#1a1a2e'}}>{email}</strong></p>
          <div style={{background:'#f8f7fc',borderRadius:14,padding:'12px 14px',marginBottom:18,fontSize:12,color:'rgba(26,26,46,.6)',lineHeight:1.6}}>
            {userCat==='gas'&&'✓ Gas tracker opens immediately — no extra steps'}
            {userCat==='driver'&&"✓ Gas tracker ready. We'll ask 2 questions to personalize your deductions."}
            {userCat==='freelancer'&&"✓ Gas tracker unlocked. We'll set up your deduction dashboard."}
            {userCat==='business'&&"✓ Gas tracker unlocked. Quick 2-min setup for your compliance modules."}
          </div>
          <input type="password" placeholder="Create a password (8+ characters)" value={password} onChange={e=>{setPassword(e.target.value);setError('')}} onKeyDown={e=>e.key==='Enter'&&password.length>=8&&handleCreate()} autoFocus style={{width:'100%',padding:'13px 16px',background:'#f8f7fc',border:`1.5px solid ${error?'#ff453a':'rgba(0,0,0,.1)'}`,borderRadius:14,fontSize:15,color:'#1a1a2e',outline:'none',fontFamily:"'DM Sans',sans-serif",marginBottom:error?6:10}}/>
          {error&&<div style={{fontSize:12,color:'#ff453a',marginBottom:10}}>{error}</div>}
          <button onClick={handleCreate} disabled={password.length<8||loading} style={{width:'100%',padding:14,background:password.length>=8&&!loading?'linear-gradient(135deg,#ff3b30,#ff6b35)':'rgba(255,59,48,.2)',color:'#fff',border:'none',borderRadius:100,fontSize:15,fontWeight:700,cursor:password.length>=8&&!loading?'pointer':'not-allowed',fontFamily:"'DM Sans',sans-serif",marginBottom:12}}>
            {loading?'Creating account...':userCat==='gas'?'Create account & open Gas Tracker →':'Create account & continue →'}
          </button>
          ...
          <p style={{fontSize:11,color:'rgba(26,26,46,.3)',textAlign:'center',marginTop:14,lineHeight:1.6}}>By creating an account you agree to our <a href="/terms" style={{color:'#ff3b30'}}>Terms</a> and <a href="/privacy" style={{color:'#ff3b30'}}>Privacy Policy</a>.</p>
        </>}
      </div>
    </div>
  )
      ...
      <div className="page">
        ...
        <section className="hero">
          ...
          <div className="hero-actions">
            <button onClick={() => setShowModal(true)} className="btn-primary">Start Free — No Card Needed →</button>
        ...
        <div className="stats-bar">
          <div className="stats-inner">
            {[{val:"$8,736",suffix:"",label:"Avg annual deductions missed by drivers"},{val:"145",suffix:"x",label:"Average ROI for Business Pass users"},{val:"3,200+",suffix:"",label:"Regulatory updates tracked per month"},{val:"Free",suffix:"",label:"To start — upgrade when you're ready"}].map((s,i)=>(
        ...
        <section className="modules-section" id="modules">
          ...
          <div className="modules-grid">
            <div className="module-card featured active" onClick={() => setShowModal(true)}>
              ...
              <br/><span className="card-cta">Open Gas Tracker →</span>
            </div>
            {[
              {icon:"📋",color:"linear-gradient(135deg,#0a84ff,#30a0ff)",title:"Regulatory Updates",desc:"IRS changes, OSHA rules, labor laws filtered to your industry. Never miss a compliance change.",meta:"IRS · OSHA · DOL · FDA"},
              {icon:"🌐",color:"linear-gradient(135deg,#ff9f0a,#ffb340)",title:"Tariff Intelligence",desc:"Live import/export tariff rates. See how changes affect your cost of goods in real time.",meta:"China · Mexico · Canada · EU"},
              {icon:"🧾",color:"linear-gradient(135deg,#30d158,#4cd964)",title:"Deduction Teller",desc:"Enter monthly expenses — we tell you what's deductible and how much you save at tax time.",meta:"Average user finds $5,760/yr unclaimed"},
              {icon:"📊",color:"linear-gradient(135deg,#bf5af2,#da8fff)",title:"Assets & Liabilities",desc:"Track net worth and debt ratio. Generate a one-page balance sheet your bank actually wants.",meta:"PDF export · Bank-ready format"},
            ].map((m,i)=>(
        ...
        <section className="cta-section">
          <div className="cta-card">
            ...
            <p>Take 60 seconds to set up your profile. We'll personalize your entire dashboard to your type — automatically.</p>
            <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
              <button onClick={() => setShowModal(true)} className="btn-primary" style={{fontSize:16,padding:"16px 36px",border:'none'}}>Create Free Account →</button>
              <Link href="/login" className="btn-secondary" style={{fontSize:16,padding:"16px 28px"}}>Log In</Link>
            </div>
            <div style={{marginTop:24,fontSize:12,color:"var(--ink-3)"}}>Free forever for basic use · No credit card required</div>
          </div>
        </section>
