...
function GCIcon({ size = 36 }: { size?: number }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
      ...
      <text x="256" y="350" fontFamily="'Arial Black',Arial,sans-serif"
        fontSize="220" fontWeight="900" fill="white" textAnchor="middle"
        letterSpacing="-14">GC</text>
    </svg>
  )
}
...
function QuickSignupModal({ onClose }: { onClose: () => void }) {
  ...
  // ── Promo code state — lives here inside the modal ──
  ...
  const validatePromo = async () => {
    ...
    if (err || !data || data.uses_count >= data.max_uses) {
      setPromoValid(false)
      return
    }
    setPromoValid(true)
    setPromoDays(data.trial_days)
  }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // 2. Calculate trial end date — 7 days default, more if promo applied
      ...
      // 4. If promo code used — record it and increment usage
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:9999,display:'flex',alignItems:'center',
      justifyContent:'center',background:'rgba(0,0,0,.45)',backdropFilter:'blur(10px)',
      WebkitBackdropFilter:'blur(10px)',padding:24}}>
      <div style={{background:'#fff',borderRadius:26,padding:'36px 32px',maxWidth:420,
        width:'100%',boxShadow:'0 32px 80px rgba(0,0,0,.18)',
        fontFamily:"'DM Sans',system-ui,sans-serif",position:'relative',
        animation:'modalPop .35s cubic-bezier(.34,1.56,.64,1)'}}>

        {/* Close */}
        <button onClick={onClose} style={{position:'absolute',top:14,right:14,width:32,
          height:32,borderRadius:'50%',background:'rgba(0,0,0,.06)',border:'none',
          cursor:'pointer',fontSize:14,color:'rgba(26,26,46,.5)',display:'flex',
        ...
        {step === 'gate' && <>
          ...
          <h2 style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:800,
            letterSpacing:-.5,textAlign:'center',color:'#1a1a2e',marginBottom:8}}>
            Start your free trial
          </h2>
          <p style={{fontSize:14,color:'rgba(26,26,46,.55)',textAlign:'center',
            lineHeight:1.65,marginBottom:22}}>
            7 days full access — no credit card needed. $4.99/mo after trial.
          </p>
          ...
          <div style={{background:'#f8f7fc',borderRadius:14,padding:'14px 16px',marginBottom:22}}>
            {[
              ...
              {icon:'🔔', text:'Price drop alerts via email'},
            ].map((f,i) => (
              <div key={i} style={{display:'flex',alignItems:'center',gap:10,fontSize:13,
                color:'rgba(26,26,46,.75)',marginBottom:i<3?9:0}}>
          ...
          <button onClick={handleContinue} style={{width:'100%',padding:14,
            background:email.trim()?'linear-gradient(135deg,#ff3b30,#ff6b35)':'rgba(255,59,48,.2)',
            color:'#fff',border:'none',borderRadius:100,fontSize:15,fontWeight:700,
            cursor:email.trim()?'pointer':'not-allowed',fontFamily:"'DM Sans',sans-serif"}}>
            Continue — it's free →
          </button>

          <div style={{display:'flex',alignItems:'center',gap:10,margin:'14px 0',
            color:'rgba(26,26,46,.3)',fontSize:12}}>
          ...
          <div style={{textAlign:'center',fontSize:13,color:'rgba(26,26,46,.45)'}}>
            ...
            <Link href="/login" style={{color:'#ff3b30',fontWeight:600,textDecoration:'none'}}
              onClick={onClose}>Log in →</Link>
          </div>
          <p style={{fontSize:11,color:'rgba(26,26,46,.3)',textAlign:'center',
        ...
        {/* ── STEP 2: User type ── */}
        {step === 'type' && <>
          <h2 style={{fontFamily:"'Sora',sans-serif",fontSize:21,fontWeight:800,
            letterSpacing:-.5,color:'#1a1a2e',marginBottom:6}}>One quick question</h2>
          <p style={{fontSize:13,color:'rgba(26,26,46,.5)',marginBottom:20}}>
            ...
            {USER_TYPES.map(t => (
              <button key={t.id} onClick={()=>{setUserCat(t.id);setStep('details')}}
                style={{display:'flex',alignItems:'center',gap:14,padding:'13px 16px',
                  background:'#f8f7fc',border:'1.5px solid rgba(0,0,0,.08)',
                  borderRadius:14,cursor:'pointer',textAlign:'left',
                  fontFamily:"'DM Sans',sans-serif"}}>
                <div style={{width:42,height:42,borderRadius:12,background:`${t.color}18`,
                  display:'flex',alignItems:'center',justifyContent:'center',
                  fontSize:19,flexShrink:0}}>{t.icon}</div>
                <div style={{flex:1}}>
          ...
          <button onClick={()=>setStep('gate')} style={{background:'none',border:'none',
            color:'rgba(26,26,46,.4)',fontSize:13,cursor:'pointer',
            fontFamily:"'DM Sans',sans-serif",padding:0}}>← Back</button>
        </>}

        {/* ── STEP 3: Password + promo ── */}
        {step === 'details' && <>
          ...
          <h2 style={{fontFamily:"'Sora',sans-serif",fontSize:21,fontWeight:800,
            letterSpacing:-.5,color:'#1a1a2e',marginBottom:4}}>Almost done</h2>
          <p style={{fontSize:13,color:'rgba(26,26,46,.5)',marginBottom:16}}>
            Creating account for{' '}
            <strong style={{color:'#1a1a2e'}}>{email}</strong>
          </p>

          {/* Trial info box */}
          <div style={{background:'#f8f7fc',borderRadius:14,padding:'12px 14px',
            marginBottom:16,fontSize:12,color:'rgba(26,26,46,.6)',lineHeight:1.6}}>
            ...
            <div style={{display:'flex',gap:8}}>
              <input
                ...
                style={{
                  ...
                  background:'#f8f7fc',
                  border:`1.5px solid ${promoValid===true?'#30d158':promoValid===false?'#ff453a':'rgba(0,0,0,.1)'}`,
                  borderRadius:12, fontSize:14, color:'#1a1a2e',
                  outline:'none', fontFamily:"'DM Sans',sans-serif",
                  letterSpacing:2, textTransform:'uppercase',
                }}
              />
              <button onClick={validatePromo} style={{padding:'11px 16px',
                background:'rgba(0,0,0,.06)',border:'none',borderRadius:12,
                fontSize:13,fontWeight:600,cursor:'pointer',
                fontFamily:"'DM Sans',sans-serif",color:'#1a1a2e',whiteSpace:'nowrap'}}>
                Apply
              </button>
            </div>
            {promoValid === true && (
              <div style={{fontSize:12,color:'#30d158',marginTop:6,fontWeight:600}}>
                ✓ Code applied — {promoDays} days free!
              </div>
            )}
            {promoValid === false && (
          ...
          {/* Password */}
          <input type="password" placeholder="Create a password (8+ characters)"
            value={password}
          ...
          <button onClick={handleCreate} disabled={password.length<8||loading}
            style={{width:'100%',padding:14,
              background:password.length>=8&&!loading
                ?'linear-gradient(135deg,#ff3b30,#ff6b35)':'rgba(255,59,48,.2)',
              color:'#fff',border:'none',borderRadius:100,fontSize:15,fontWeight:700,
          ...
          <button onClick={()=>setStep('type')} style={{background:'none',border:'none',
            color:'rgba(26,26,46,.4)',fontSize:13,cursor:'pointer',
            fontFamily:"'DM Sans',sans-serif",padding:0}}>← Back</button>
          ...
          <p style={{fontSize:11,color:'rgba(26,26,46,.3)',textAlign:'center',
            ...
            <a href="/terms" style={{color:'#ff3b30'}}>Terms</a> &{' '}
            <a href="/privacy" style={{color:'#ff3b30'}}>Privacy</a>
          </p>
        </>}
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .hero-trust{display:flex;align-items:center;justify-content:center;gap:28px;margin-top:52px;flex-wrap:wrap;animation:fadeUp 0.8s cubic-bezier(0.34,1.56,0.64,1) 0.5s both}
        ...
        .module-card.active{border-color:rgba(255,59,48,0.25);box-shadow:0 2px 8px rgba(255,59,48,0.08),0 8px 24px rgba(255,59,48,0.06)}
      ...
      <div className="page">
        ...
        <section className="hero">
          ...
          <p className="hero-sub">
            Real-time gas prices, regulatory updates, tariff tracking, and tax
            deductions — all in one platform built for gig workers, freelancers,
          ...
          <div style={{marginTop:14,fontSize:13,color:'var(--ink-3)'}}>
            No credit card needed · $4.99/mo after trial · Cancel anytime
          </div>
        ...
        <div className="stats-bar">
          <div className="stats-inner">
            {[
              ...
              {val:"$4.99",  suffix:"",  label:"Per month after your free trial"},
            ].map((s,i) => (
          ...
          <div className="modules-grid">
            <div className="module-card featured active" onClick={() => setShowModal(true)}>
              ...
              <div className="card-desc">
                Real-time gas prices at stations near you. Compare grades, track
                7-day trends, and calculate your exact IRS mileage deduction —
                updated every hour from EIA.gov.
              </div>
              <div className="card-meta">$3.07 near you right now · ↓ Prices falling</div>
              <br/>
              <span className="card-cta">Start Free Trial →</span>
            </div>

            {[
              {icon:"📋", color:"linear-gradient(135deg,#0a84ff,#30a0ff)", title:"Regulatory Updates",
               desc:"IRS changes, OSHA rules, labor laws filtered to your industry.", meta:"IRS · OSHA · DOL · FDA"},
              {icon:"🌐", color:"linear-gradient(135deg,#ff9f0a,#ffb340)", title:"Tariff Intelligence",
               desc:"Live import/export tariff rates. See how changes affect your costs.", meta:"China · Mexico · Canada · EU"},
              {icon:"🧾", color:"linear-gradient(135deg,#30d158,#4cd964)", title:"Deduction Teller",
               desc:"Enter monthly expenses — we find what's deductible and what you're missing.", meta:"Average user finds $5,760/yr"},
              {icon:"📊", color:"linear-gradient(135deg,#bf5af2,#da8fff)", title:"Assets & Liabilities",
               desc:"Track net worth and generate a bank-ready one-page balance sheet.", meta:"PDF export · Bank-ready"},
            ].map((m,i) => (
        ...
        <section className="cta-section">
          <div className="cta-card">
            ...
            <p>
              Take 60 seconds to set up your profile. 7 days free —
              no card needed. $4.99/mo after trial, cancel anytime.
            </p>
            <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
              <button onClick={() => setShowModal(true)} className="btn-primary"
                style={{fontSize:16,padding:"16px 36px",border:'none'}}>
                Start Free Trial →
              </button>
            ...
            <div style={{marginTop:16,fontSize:12,color:"var(--ink-3)"}}>
              No credit card · 7-day free trial · $4.99/mo after · Cancel anytime
            </div>
          </div>
        </section>
