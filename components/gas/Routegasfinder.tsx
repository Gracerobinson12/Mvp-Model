...
function RouteMap({ stations, grade, selectedId, onSelect, origin, destination, routePolyline, mapKey }) {
  ...
  const initMap = useCallback((L) => {
    ...
    if (routePolyline.length > 1) {
      const line = L.polyline(routePolyline.map(p => [p.lat,p.lng]), { color:'#ff3b30', weight:5, opacity:.75, lineCap:'round', lineJoin:'round' }).addTo(map)
      map.fitBounds(line.getBounds(), { padding: [48,48] })
    }

    if (origin) {
      L.marker([origin.lat,origin.lng], { icon: L.divIcon({ className:'', iconSize:[24,24], iconAnchor:[12,12],
    ...
    if (destination) {
      L.marker([destination.lat,destination.lng], { icon: L.divIcon({ className:'', iconSize:[30,38], iconAnchor:[15,38],
    ...
    stations.forEach(st => {
      ...
      const m = L.marker([st.lat,st.lng], { icon: L.divIcon({ className:'', iconSize:[80,52], iconAnchor:[40,52], html: makePin(price, st.id===cheapest?.id, false) }) })
      .addTo(map).on('click', () => onSelect(st.id))
      m.bindPopup(`<div style="font-family:-apple-system,system-ui,sans-serif;min-width:170px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-bottom:8px">
          ${['Regular','Mid','Premium','Diesel'].map(g=>`<div style="background:${g===grade?'rgba(255,59,48,.08)':'rgba(0,0,0,.04)'};border-radius:7px;padding:5px 8px">
  ...
  useEffect(() => {
    ...
    if (!document.querySelector('#leaflet-js')) {
      ...
      const wait = setInterval(() => { if ((window as any).L) { clearInterval(wait); boot((window as any).L) } }, 80)
    }
  ...
  useEffect(() => {
    ...
    stations.forEach(st => {
      ...
      m.setIcon(L.divIcon({ className:'', iconSize:[80,52], iconAnchor:[40,52], html: makePin(st[gk(grade)], st.id===best?.id, st.id===selectedId) }))
    })
  ...
  useEffect(() => {
    ...
    if (st) { mapRef.current.panTo([st.lat,st.lng], { animate:true, duration:.5 }); setTimeout(() => mksRef.current[selectedId]?.openPopup(), 600) }
  }, [selectedId])
  ...
  return (
    <>
      <style>{`
        ...
        .leaflet-control-zoom a{background:rgba(255,255,255,.9)!important;backdrop-filter:blur(12px)!important;color:rgba(26,26,46,.55)!important;border:1px solid rgba(0,0,0,.1)!important;border-radius:9px!important;margin-bottom:3px!important;display:block!important;width:30px!important;height:30px!important;line-height:30px!important;text-align:center!important}
...
export default function RouteGasFinder({ userCoords, basePrice = 3.15, isDark = true }) {
  ...
  useEffect(() => {
    ...
    const sc = document.createElement('script'); sc.id='gmaps-places'; sc.async=true
  ...
  const fetchSuggestions = useCallback((input) => {
    ...
    const opts: any = { input, ...(userCoords ? { locationBias: { center: new google.maps.LatLng(userCoords.lat, userCoords.lng), radius: 80000 } } : {}) }
  ...
  const resolvePlace = useCallback((placeId, label) => {
  ...
  const geocodeText = useCallback((text) => {
    return new Promise<LatLng>((resolve, reject) => {
      ...
      new google.maps.Geocoder().geocode({ address: text }, (results, status) => {
  ...
  const getDirections = useCallback((origin, dest) => {
    return new Promise<{ polyline: LatLng[]; miles: number; minutes: number }>((resolve, reject) => {
      const google = (window as any).google
      if (!google?.maps) { reject('Google Maps not loaded'); return }
      new google.maps.DirectionsService().route({
        ...
        const polyline = decodePoly(result.routes[0].overview_polyline)
        resolve({ polyline, miles: parseFloat((leg.distance.value/1609.34).toFixed(1)), minutes: Math.round(leg.duration.value/60) })
      })
    })
  }, [])
  ...
  const pickSuggestion = async (s) => {
    ...
    try { setDestCoords(await resolvePlace(s.placeId, s.description)) }
  ...
  const searchRoute = useCallback(async () => {
    ...
    try {
      ...
      let dest = destCoords
      if (!dest) { try { dest = await resolvePlace('', query) } catch { dest = await geocodeText(query) }; setDestCoords(dest); setDestLabel(query) }
      ...
      const built: RouteStation[] = routePoints.map((pt,i) => {
        const distAlongRoute = parseFloat(((i+1)/(routePoints.length+1)*miles).toFixed(1))
        const detour = parseFloat((Math.random()*0.6+0.1).toFixed(1))
        return {
          id: i+1, name: stationNames[Math.floor(Math.random()*stationNames.length)],
          address: `Near mile ${distAlongRoute} of your route`,
          lat: pt.lat+(Math.random()-0.5)*0.008, lng: pt.lng+(Math.random()-0.5)*0.008,
          ...simulatePrices(basePrice), distOnRoute: distAlongRoute, detourMiles: detour,
    ...
    } catch (e: any) { setError(e?.message || String(e) || 'Could not find route.') }
    setLoadStep(''); setLoading(false)
  }, [query, destCoords, destLabel, userCoords, basePrice, resolvePlace, geocodeText, getDirections])

  const handleKeyDown = (e) => { if (e.key==='Enter') { setShowSuggest(false); searchRoute() }; if (e.key==='Escape') setShowSuggest(false) }
  ...
  return (
    <>
      <style>{`
        ...
        .suggest-item:hover{background:rgba(255,59,48,.07)!important}
      `}</style>

      <div style={{ background:S.bg, border:`1px solid ${S.bdr}`, borderRadius:20, backdropFilter:'blur(24px)', WebkitBackdropFilter:'blur(24px)', overflow:'visible', marginBottom:14, position:'relative' }}>
        ...
        <div style={{ padding:'0 20px 16px', position:'relative' }}>
          <div style={{ display:'flex', gap:8, alignItems:'stretch' }}>
            ...
            <div style={{ flex:1, position:'relative' }}>
              <div style={{ display:'flex', alignItems:'center', background:S.inputBg, border:`1.5px solid ${showSuggest||query?'#ff3b30':S.inputBdr}`, borderRadius:13, padding:'0 12px', gap:8, boxShadow:showSuggest||query?'0 0 0 3px rgba(255,59,48,.1)':'none', transition:'border-color .2s,box-shadow .2s' }}>
              ...
              {showSuggest && suggestions.length > 0 && (
                <div ref={suggestRef} style={{ position:'absolute', top:'calc(100% + 6px)', left:0, right:0, background:isDark?'rgba(18,18,22,.97)':'rgba(255,255,255,.98)', border:`1px solid ${S.bdr}`, borderRadius:14, boxShadow:'0 8px 32px rgba(0,0,0,.2)', backdropFilter:'blur(40px)', zIndex:9999, overflow:'hidden', animation:'routeIn .2s ease both' }}>
                  {suggestions.map((s,i) => (
                    <div key={s.placeId} className="suggest-item" onClick={() => pickSuggestion(s)}
                      style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 14px', cursor:'pointer', borderBottom:i<suggestions.length-1?`1px solid ${S.bdr}`:'none', transition:'background .15s' }}>
          ...
          {loading && loadStep && (
            <div style={{ marginTop:8, fontSize:11, color:'#ff3b30', fontWeight:500, display:'flex', alignItems:'center', gap:6 }}>
              <span style={{ animation:'spin 1s linear infinite', display:'inline-block', fontSize:13 }}>⟳</span>{loadStep}
            </div>
          )}
          {error && !loading && (
            <div style={{ marginTop:8, fontSize:12, color:'#ff453a', fontWeight:500, background:'rgba(255,69,58,.08)', border:'1px solid rgba(255,69,58,.2)', borderRadius:10, padding:'8px 12px' }}>⚠️ {error}</div>
          )}
        </div>
          ...
          <div style={{ textAlign:'center', padding:'28px 20px 36px' }}>
            ...
            <div style={{ display:'flex', justifyContent:'center', gap:8, marginTop:16, flexWrap:'wrap' }}>
              {['Atlanta Airport','Walmart Supercenter','Nashville, TN','Miami Beach'].map(ex => (
                <button key={ex} onClick={() => { setQuery(ex); handleQueryChange(ex); inputRef.current?.focus() }}
                  style={{ padding:'5px 12px', background:S.inputBg, border:`1px solid ${S.inputBdr}`, borderRadius:100, fontSize:11, fontWeight:500, color:S.text2, cursor:'pointer', fontFamily:"'DM Sans',system-ui,sans-serif" }}>
                  {ex}
                </button>
              ))}
            </div>
          ...
          <div style={{ padding:'0 20px 16px' }}>
            {[1,2,3].map(i => (
              <div key={i} style={{ height:64, background:S.inputBg, borderRadius:13, marginBottom:8, position:'relative', overflow:'hidden' }}>
                <div style={{ position:'absolute', top:0, bottom:0, width:'55%', background:`linear-gradient(90deg,transparent,rgba(255,59,48,.07),transparent)`, animation:`shimmer 1.4s ease-in-out ${i*0.15}s infinite` }}/>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ animation:'routeIn .4s cubic-bezier(.34,1.56,.64,1)' }}>
            ...
            {routeInfo && cheapest && (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, padding:'0 16px', marginBottom:14 }}>
                {[
                  ...
                  { label:'Fuel Est',      val:`$${routeFuelCost}`,            sub:'full route at best',   color:S.text   },
                ].map(s => (
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'0 16px', marginBottom:14 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', background:S.inputBg, border:`1px solid ${S.inputBdr}`, borderRadius:10, padding:'5px 10px', gap:5, width:90 }}>
                <input type="number" value={tankSize} min={1} max={35} onChange={e => setTankSize(+e.target.value)}
                  style={{ background:'transparent', border:'none', outline:'none', color:'#ff3b30', fontSize:15, fontWeight:700, width:'100%', fontFamily:"'DM Sans',system-ui,sans-serif" }}/>
            ...
            <div style={{ padding:'0 16px 6px' }}>
              ...
              {sorted.map((st,i) => {
                ...
                return (
                  <div key={st.id} onClick={() => setSelId(p => p===st.id?null:st.id)}
                    style={{ display:'flex', alignItems:'center', gap:11, padding:'12px 14px', borderRadius:13, marginBottom:7, cursor:'pointer', border:`1.5px solid ${isSel?'#ff3b30':isBest?'rgba(48,209,88,.3)':S.bdr}`, background:isSel?'rgba(255,59,48,.06)':isBest?'rgba(48,209,88,.05)':S.inputBg, transition:'all .18s', animation:`routeIn .3s cubic-bezier(.34,1.56,.64,1) ${i*0.06}s both` }}>
            ...
            {sel && (
              <div style={{ margin:'0 16px 20px', padding:'16px', background:'linear-gradient(135deg,rgba(255,59,48,.09),rgba(255,59,48,.03))', border:'1px solid rgba(255,59,48,.22)', borderRadius:16, animation:'routeIn .3s ease' }}>
                ...
                <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                  <button onClick={() => { const o=userCoords,d=destCoords,wp=sel; if(o&&d&&wp) window.open(`https://www.google.com/maps/dir/?api=1&origin=${o.lat},${o.lng}&destination=${d.lat},${d.lng}&waypoints=${wp.lat},${wp.lng}&travelmode=driving`) }}
                    style={{ flex:1, minWidth:150, padding:'11px 16px', background:'linear-gradient(135deg,#ff3b30,#ff6b35)', color:'#fff', border:'none', borderRadius:12, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:"'DM Sans',system-ui,sans-serif", boxShadow:'0 4px 14px rgba(255,59,48,.35)' }}>
                    Add as Waypoint in Google Maps →
                  </button>
                  <button onClick={() => window.open(`https://maps.google.com/?q=${sel.lat},${sel.lng}`)}
                    style={{ padding:'11px 16px', background:S.inputBg, color:S.text2, border:`1px solid ${S.inputBdr}`, borderRadius:12, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:"'DM Sans',system-ui,sans-serif" }}>
                    View Station
                  </button>
                </div>
              </div>
